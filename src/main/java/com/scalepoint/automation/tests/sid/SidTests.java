...
public class SidTests extends BaseTest {
     ...
     */
    @Test(dataProvider = "testDataProvider", description = "ECC-3025 It's possible to calculate shop distance in Settlement dialog", enabled = false)
    ...
    public void ecc3025_cashCompensationWithoutDepNoVoucher(User user, Claim claim, Category category) {
        login(user, AdminPage.class)
                .createPsModelWithCategoryAndEnable(category, "Standard Model")
                .logout();
